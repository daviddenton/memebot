...
var Custombox = function (a, b, c) {
    ...
    var d = {create: document.createElement, settings: [], overlay: [], wrapper: [], container: [], modal: [], content: [], inline: [], size: [], close: [], open: [], scroll: [], item: -1}, e = {target: null, id: null, cache: !1, escKey: !0, zIndex: "auto", overlay: !0, overlayColor: "#000", overlayOpacity: .8, overlayClose: !0, overlaySpeed: 300, overlayEffect: "auto", width: null, effect: "fadein", position: "center, center", animation: null, speed: 500, open: null, complete: null, close: null}, f = {oldIE: navigator.appVersion.indexOf("MSIE 8.") > -1 || navigator.appVersion.indexOf("MSIE 9.") > -1, oldMobile: /(iPhone|iPad|iPod)\sOS\s6/.test(navigator.userAgent), overlay: {perspective: ["letmein", "makeway", "slip"], together: ["corner", "slidetogether", "scale", "door", "push", "contentscale", "simplegenie", "slit", "slip"]}, modal: {position: ["slide", "flip", "rotate"]}}, g = {init: function () {this.merge().built().check().binds()}, merge: function () {return d.settings.push(h.extend({}, e, d.options)), delete d.options, d.item = d.settings.length - 1, "auto" === d.settings[d.item].overlayEffect && (d.settings[d.item].overlayEffect = d.settings[d.item].effect), this}, built: function () {
        ...
        if (d.content[d.item].style.width = "auto", d.modal[d.item].appendChild(d.content[d.item]), d.content[d.item].offsetHeight >= a.innerHeight)d.container[d.item].style.marginTop = "5%", d.container[d.item].style.marginBottom = "5%"; else {
            var i;
        ...
        if (d.settings[d.item].overlay)if (f.overlay.perspective.indexOf(d.settings[d.item].overlayEffect) > -1 || f.overlay.together.indexOf(d.settings[d.item].overlayEffect) > -1 ? d.overlay[d.item].classList.add("custombox-overlay-open") : d.overlay[d.item].style.opacity = d.settings[d.item].overlayOpacity, d.main.classList.add("custombox-container-open"), f.overlay.together.indexOf(d.settings[d.item].overlayEffect) > -1 || f.oldIE)d.wrapper[d.item].classList.add("custombox-modal-open"); else {
            var a = function () {d.overlay[d.item].removeEventListener("transitionend", a), d.wrapper[d.item].classList.add("custombox-modal-open")};
        ...
        var c = this;
        d.settings[d.item].escKey && (b.onkeydown = function (b) {b = b || a.event, 27 === b.keyCode && c.close()}), d.settings[d.item].overlayClose && d.wrapper[d.item].addEventListener("click", function (a) {a.target === d.wrapper[d.item] && c.close()}, !1), a.addEventListener("onorientationchange"in a ? "orientationchange" : "resize", function () {c.responsive()}, !1);
        var e = function () {
            if (!d.inline[d.item])for (var a = 0, c = d.modal[d.item].getElementsByTagName("script"), e = c.length; e > a; a++)new Function(c[a].text)();
        ...
        }, g = function () {e(), d.modal[d.item].removeEventListener("transitionend", g)};
        f.oldIE || f.oldMobile ? setTimeout(function () {e()}, d.settings[d.item].overlaySpeed) : "slit" !== d.settings[d.item].effect ? d.modal[d.item].addEventListener("transitionend", g, !1) : d.modal[d.item].addEventListener("animationend", g, !1)
    }, close: function (e) {
        var g;
        if ("first" === e)g = 0; else if ("undefined" != typeof e) {
            for (var h = 0, i = d.settings.length; i > h; h++)if (d.settings[h].id == e) {
                g = h;
                break
            }
        ...
        for (var b, e = 0, g = d.container.length; g > e; e++) {
            ...
            if (d.content[e].offsetHeight >= a.innerHeight)d.container[e].style.marginTop = "5%", d.container[e].style.marginBottom = "5%"; else {
    ...
    }, error: function () {alert("Error to load this target: " + d.settings[d.item].target)}}, h = {zIndex: function () {
        ...
        var b = 0;
        if (isNaN(d.settings[d.item].zIndex))for (var c = 0, e = document.getElementsByTagName("*"), f = e.length; f > c; c += 1) {
    ...
    return{open: function (a) {d.options = a, g.init()}, close: function (a) {g.close(a)}}
