...
exports.MemebotServer = function (config, log) {
    ...
    app.get('/search', function (request, response) {
        var currentPageIndex = parseInt(request.query.page || 0);
        withCatch(response, memeApi.search(request.query.q, currentPageIndex).then(function (results) {
            response.render('search', { title: 'Search:' + request.query.q,
                memeMappingsUrl: memeMappingsUrl,
                results: results,
                query: request.query.q,
                previousLink: currentPageIndex == 0 ? undefined : '/search?q=' + request.query.q + '&page=' + (currentPageIndex - 1),
                nextLink: '/search?q=' + request.query.q + '&page=' + (currentPageIndex + 1)
            });
        }));
    });
    ...
    return {
        start: function () {
            app.listen(port, function () {
                log("Memebot is running at localhost:" + port);
            });
        }
    };
};
